...
3. **Owner authorization is external to the certified runtime.** The reusable runtime stores only the static single-campaign policy. A fresh `authorization_id` is carried by the launch request and consumed into append-only state before the single workflow dispatch. Issuing another PAPER authorization therefore does not mutate the certified runtime SHA.
