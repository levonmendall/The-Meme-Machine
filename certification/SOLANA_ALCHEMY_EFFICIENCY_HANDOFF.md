...
## Smoke publisher capacity repair (prepared after 450ef83c)

Combined run 35521414965 passed hosted gates and launched concurrent smoke. At 423s overlap the last public view showed zero restarts, reconciled accounting in all lanes, Pump 7 complete observations, Meteora 4 economic vectors, Pons 139 evaluated / 2 qualified / 1 open paper position, Ramses 1 completed scan. No settlements were established by that snapshot. The snapshot became stale and must not be used to infer later health.

The captured view was 59,569 bytes, including 25 completed Pons provider sessions. Two further sessions deterministically exceed the existing 60,000-byte publication guard. The prepared repair publishes full-session totals plus two recent session samples; raw session history is unmodified. The captured growth case now fits in 38KB. A 4,000-session regression proves bounded publication, complete totals, retained accounting and no mutation of raw data. Immutable-cache category labels are explicitly allowlisted. Smoke is labeled as 600s engineering preflight rather than four-hour certification.

Validation: 64 supervisor tests, all 1,181 lane tests, three resource gates, exact-source/policy/config/overlay integrity passed. Transcripts: results/solana-publisher-validation/. Terminal evidence must confirm the live publisher failure and determine all lifecycle/accounting outcomes before replacement launch.
