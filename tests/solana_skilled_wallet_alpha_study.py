...
import bisect,io,json,math,random,statistics,urllib.parse,urllib.request
...
def pub(url):
 ...
 with urllib.request.urlopen(req,timeout=30) as r:return json.loads(r.read(4_000_000))
def cohort():
 b30=made({"period":"30d","min_tokens":MIN_TOKENS,"sort":"roi","exclude_bots":"true","limit":100})
 b7=made({"period":"7d","min_tokens":MIN_TOKENS,"sort":"roi","exclude_bots":"true","limit":100})
 rows=(b30.get("leaderboard") or b30.get("data") or [])[:COHORT_N]
 s7={x.get("wallet") for x in (b7.get("leaderboard") or b7.get("data") or []) if x.get("wallet")}
 out=[]
 for i,r in enumerate(rows):
  if not r.get("wallet"):continue
  out.append({"wallet":r["wallet"],"rank_30d":r.get("rank",i+1),"roi_30d":r.get("roi"),"net_pnl_sol_30d":r.get("net_pnl_sol"),
              "tokens_traded_30d":r.get("tokens_traded"),"win_rate_30d":r.get("win_rate"),"persistent_7d_top100":r["wallet"] in s7})
 return out
