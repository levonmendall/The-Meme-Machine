...
class RamsesActiveWideMakerV2Tests(unittest.TestCase):
    ...
    def test_inside_holds_without_fee_reserve(self):
        result=rebalance_decision(
            ...
            candidates=[candidate(overlap=.75)],
        )
        self.assertEqual(result["action"],"hold")

    def test_inside_compounds_only_with_economic_broad_overlap(self):
        result=rebalance_decision(
            ...
            unwind_deteriorated=False,
            candidates=[
                candidate(width=65,after=60,overlap=.75),
                candidate(width=90,after=80,overlap=.60),
                candidate(width=120,after=80,overlap=.80),
            ],
        )
    ...
    def test_watch_band_prohibits_partial_shift(self):
        result=rebalance_decision(
            ...
            unwind_deteriorated=False,
            candidates=[candidate(overlap=.3)],
        )
        self.assertEqual(result["action"],"watch")
        self.assertEqual(result["reason"],"hysteresis_no_partial_shift")

    def test_recenter_band_requires_fresh_broad_full_reset(self):
        result=rebalance_decision(
            ...
            unwind_deteriorated=False,
            candidates=[
                candidate(width=42,after=100,overlap=0.0),
                candidate(width=90,after=20,overlap=.25),
                candidate(width=120,after=30,overlap=.05),
                candidate(width=150,after=30,overlap=.05),
            ],
        )
        self.assertEqual(result["action"],"recenter")
        self.assertEqual(result["replacement"]["width_bins"],120)
    ...
    def test_recenter_exits_when_no_valid_replacement(self):
        result=rebalance_decision(
            ...
            unwind_deteriorated=False,
            candidates=[
                candidate(width=42,overlap=0.0),
                candidate(width=90,overlap=.3),
                candidate(width=120,stress=0,overlap=.05),
            ],
        )
        self.assertEqual(result["action"],"exit")
    ...
    def test_overrun_never_chases(self):
        result=rebalance_decision(
            ...
            inventory_risk_quote=0,remaining_fee_reserve_quote=500,
            unwind_deteriorated=False,
            candidates=[candidate(width=200,after=100,stress=100,overlap=0)],
        )
    ...
    def test_candidate_hard_gates(self):
        ...
        self.assertTrue(candidate_eligible(candidate(overlap=.10),mode="recenter"))
        self.assertTrue(candidate_eligible(candidate(overlap=.50),mode="compound_resize"))

    def test_selection_prefers_edge_then_narrower_tie(self):
        choice=select_replacement([
            candidate(width=90,after=50,overlap=0),
            candidate(width=120,after=60,overlap=0),
            candidate(width=150,after=60,overlap=0),
        ],mode="recenter")
