...
def decode_add_liquidity(raw,pool):
    if len(raw)!=124 or raw[:8]!=ADD_LIQUIDITY_EVT or pump.b58(raw[8:40])!=pool:
        raise ValueError('dlmm_add_liquidity_event_layout_or_identity')
    sender=pump.b58(raw[40:72]);position=pump.b58(raw[72:104])
    amount_x,amount_y=struct.unpack_from('<QQ',raw,104)
    active=struct.unpack_from('<i',raw,120)[0]
    return dict(position=position,sender=sender,amount_x=amount_x,
                amount_y=amount_y,active=active)
...
def _remaining_accounts_info_end(raw,offset):
    if len(raw)<offset+4:
        raise Unavailable('dlmm_add_liquidity2_remaining_accounts_shape')
    count=struct.unpack_from('<I',raw,offset)[0];offset+=4
    if count>16:
        raise Unavailable('dlmm_add_liquidity2_remaining_accounts_bound')
    for _ in range(count):
        if offset>=len(raw):
            raise Unavailable('dlmm_add_liquidity2_remaining_accounts_shape')
        kind=raw[offset];offset+=1
        if kind>4:
            raise Unavailable('dlmm_add_liquidity2_remaining_accounts_kind')
        if kind==3:
            if offset>=len(raw):
                raise Unavailable('dlmm_add_liquidity2_remaining_accounts_shape')
            offset+=1
        ...
        offset+=1
    if offset!=len(raw):
        raise Unavailable('dlmm_add_liquidity2_remaining_accounts_trailing')
    return offset


def _add_liquidity2_record(raw,instruction,keys,pool,order):
    accounts=instruction.get('accounts') or []
    if len(raw)<32 or len(accounts)<14:
    ...
    if not 1<=count<=70:
        raise Unavailable('dlmm_add_liquidity2_distribution_bound')
    offset=28
    if len(raw)<offset+count*8+4:
        raise Unavailable('dlmm_add_liquidity2_distribution_shape')
    distributions=[];seen=set();sum_x=sum_y=0
    for _ in range(count):
        bid,dx,dy=struct.unpack_from('<iHH',raw,offset);offset+=8
        if bid in seen or dx>10000 or dy>10000:
            raise Unavailable('dlmm_add_liquidity2_distribution_invalid')
        seen.add(bid);sum_x+=dx;sum_y+=dy
        distributions.append(dict(
            bin_id=bid,distribution_x=dx,distribution_y=dy,
            x=amount_x*dx//10000,y=amount_y*dy//10000))
    if (amount_x and sum_x!=10000) or (amount_y and sum_y!=10000):
        raise Unavailable('dlmm_add_liquidity2_distribution_sum')
    _remaining_accounts_info_end(raw,offset)
    actual_x=sum(row['x'] for row in distributions)
    actual_y=sum(row['y'] for row in distributions)
    if actual_x and actual_y:
        raise Unavailable('dlmm_add_liquidity2_two_sided_unsupported')
    return dict(
        kind='add_liquidity2',order=list(order),events=[],
        position=keys[accounts[0]],sender=keys[accounts[9]],
        max_amount_x=amount_x,max_amount_y=amount_y,
        expected_amount_x=actual_x,expected_amount_y=actual_y,
...
def _add_liquidity_one_side_record(raw,instruction,keys,pool,order):
    """Decode the proven classic-SPL Y-side weighted one-sided deposit shape."""
    accounts=instruction.get('accounts') or []
    ...
    offset=28
    if len(raw)!=offset+count*6:
        raise Unavailable('dlmm_add_liquidity_one_side_distribution_shape')
    weights=[];seen=set()
    for _ in range(count):
        bid,weight=struct.unpack_from('<iH',raw,offset);offset+=6
        if bid in seen or weight<=0:
            raise Unavailable('dlmm_add_liquidity_one_side_distribution_invalid')
    ...
    return dict(
        ...
        user_token=keys[accounts[3]],reserve=keys[accounts[4]],
        token_mint=keys[accounts[5]],user_token_index=accounts[3],
        reserve_index=accounts[4],mint_index=accounts[5],
    )
...
def _add_liquidity_by_strategy2_record(raw,instruction,keys,pool,order):
    ...
    Strategy2 chooses its exact per-bin distribution inside the program.  We never
    infer that distribution from the strategy formula.  Instead the finalized
    terminal snapshot must prove the exact one-sided bin deltas, while the
    instruction/event/ordered transfers authenticate identity and aggregate amounts.
    """
    accounts=instruction.get('accounts') or []
    if len(raw)<105 or len(accounts)<14:
        raise Unavailable('dlmm_add_liquidity_by_strategy2_identity')
    if any(type(accounts[i]) is not int or not 0<=accounts[i]<len(keys)
           for i in range(14)):
        raise Unavailable('dlmm_add_liquidity_by_strategy2_account_index')
    if (keys[accounts[1]]!=pool
            or not _supported_token_program(keys[accounts[10]])
            or not _supported_token_program(keys[accounts[11]])
            or keys[accounts[13]]!=dlmm.PROGRAM):
        raise Unavailable('dlmm_add_liquidity_by_strategy2_identity')
    amount_x,amount_y,observed_active,max_slippage,min_bin,max_bin=struct.unpack_from(
        '<QQiiii',raw,8)
...
def _attach_effect_event(effects,kind,event,order):
    ...
    label='_or_'.join(kinds)
    candidates=[
        effect for effect in effects
        if effect.get('kind') in kinds
    ...
    if not candidates:
        raise Unavailable(f'dlmm_{label}_event_without_ordered_call')
    ...
    if len(matches)!=1:
        raise Unavailable(f'dlmm_{label}_event_ambiguous_order')
    matches[0]['events'].append(event)
    matches[0]['event_order']=list(order)
...
def _resolve_effect_event(effect,pool):
    ...
    if effect['kind']=='claim_fee2':
        ...
        effect.update(
            ...
            'add_liquidity2','add_liquidity_by_strategy2',
        ...
        if effect['kind']=='add_liquidity_one_side':
            # Proven historical shape: classic SPL token Y, all weighted bins on
            # the bid side. Each bin receives floor(amount*weight/total_weight);
            # the aggregate event/ordered transfer authenticates rounding dust.
            if event['amount_x']!=0 or event['amount_y']<=0:
                raise Unavailable('dlmm_add_liquidity_one_side_x_unsupported')
            ...
            if any(row['bin_id']>active for row in effect['weights']):
                raise Unavailable('dlmm_add_liquidity_one_side_y_distribution_side')
            total_weight=sum(row['weight'] for row in effect['weights'])
            if total_weight<=0:
                raise Unavailable('dlmm_add_liquidity_one_side_weight_sum')
            deposits=[
                dict(bin_id=row['bin_id'],x=0,
                     y=effect['max_amount']*row['weight']//total_weight)
                for row in effect['weights']]
            deposits=[row for row in deposits if row['y']]
            actual=sum(row['y'] for row in deposits)
            if actual!=event['amount_y']:
                raise Unavailable('dlmm_add_liquidity_one_side_rounding_mismatch')
            effect.update(
                amount_x=0,amount_y=actual,active=active,side='y',
            ...
            if abs(active-effect['observed_active'])>effect['max_active_bin_slippage']:
                raise Unavailable('dlmm_add_liquidity_by_strategy2_active_slippage')
            effect.update(
                amount_x=event['amount_x'],amount_y=event['amount_y'],
                active=active)
        else:
            if (event['amount_x']!=effect['expected_amount_x']
                    or event['amount_y']!=effect['expected_amount_y']):
                raise Unavailable('dlmm_add_liquidity2_event_amount_mismatch')
            deposits=[]
            for row in effect['distributions']:
                x=row['x'];y=row['y'];bid=row['bin_id']
                if not x and not y:
                    continue
                if (x and bid<=active) or (y and bid>=active):
                    raise Unavailable('dlmm_add_liquidity2_active_composition_unsupported')
                deposits.append(dict(bin_id=bid,x=x,y=y))
            if not deposits and (event['amount_x'] or event['amount_y']):
                raise Unavailable('dlmm_add_liquidity2_empty_distribution')
            effect.update(
                amount_x=event['amount_x'],amount_y=event['amount_y'],
                active=active,bin_deposits=deposits)
    else:
...
def _authenticate_add_liquidity_transfers(effects,meta,keys,ordered):
    for effect in effects:
        start=tuple(effect.get('order') or ())
        ...
        sides=(effect.get('side'),) if effect.get('kind')=='add_liquidity_one_side' \
              else ('x','y')
        for side in sides:
            if side not in ('x','y'):
                raise Unavailable('dlmm_add_liquidity_one_side_event_sides')
            amount=int(effect[f'amount_{side}'])
            one_side=effect.get('kind')=='add_liquidity_one_side'
            mint=effect['token_mint'] if one_side else effect[f'token_{side}_mint']
            source=effect['user_token_index'] if one_side else effect[f'user_{side}_index']
            destination=effect['reserve_index'] if one_side else effect[f'reserve_{side}_index']
            total=0;seen=False
            for order,instruction in ordered:
                order=tuple(order)
                if order[0]!=start[0] or not start<order<end:
                    continue
                transfer=_spl_transfer(instruction,keys)
                if transfer is None or transfer['source']!=source \
                        or transfer['destination']!=destination:
                    continue
                if transfer['mint'] is not None and transfer['mint']!=mint:
                    raise Unavailable('dlmm_add_liquidity2_transfer_wrong_token')
...
def _resolve_swap_record(record,meta,block_time,trigger_only=False):
    ...
    if trigger_only:
        ...
        return dict(
            **event,time=block_time,instruction=record['instruction'],
            execution_order=record['order'],swap_mode='trigger_only')
...
def transaction_swaps(tx,pool,terminal_adjustments=None,trigger_only=False):
    ...
    for (outer,inner),instruction in ordered:
        ...
        if raw[:8] in EXACT_SWAP:
            ...
            if positions:
                if positions!=[1]:
                    raise ValueError(
                        f'dlmm_add_liquidity2_identity:pool_positions={positions}')
                effect=_add_liquidity2_record(
                    raw,instruction,keys,pool,[outer,inner])
                effects.append(effect);current=dict(kind='effect',effect=effect)
            else:
                current=None
            ...
            if positions:
                if positions!=[1]:
                    raise ValueError(
                        f'dlmm_add_liquidity_by_strategy2_identity:pool_positions={positions}')
    ...
    for record in records:
        ...
        event=_resolve_swap_record(record,meta,tx.get('blockTime'),trigger_only=trigger_only)
    ...
    if resolved_effects:
        if any(record['target'] for record,_event in resolved) \
                and any(effect['kind']=='add_liquidity_by_strategy2'
                        for effect in resolved_effects):
            # Strategy2 bin amounts are derived from terminal deltas and cannot
            # be separated from a same-interval swap. Direct weighted adds and
            # authenticated removals are replayed in transaction order instead.
            raise Unavailable('dlmm_swap_mixed_with_external_liquidity_effect')
        add_kinds=('add_liquidity2','add_liquidity_by_strategy2',
                   'add_liquidity_one_side')
        adds=[effect for effect in resolved_effects
              if effect['kind'] in add_kinds]
        others=[effect for effect in resolved_effects
                if effect['kind'] not in add_kinds]
        if adds:
            _authenticate_add_liquidity_transfers(adds,meta,keys,ordered)
        if others:
            _authenticate_effect_transfers(others,meta,keys,ordered)
...
def _materialize_removal_effects(start,end,effects):
    ...
    if strategy_adds:
        ...
        if set(start['bins'])!=set(end['bins']):
            raise Unavailable('dlmm_add_liquidity_by_strategy2_bin_set_changed')
        item=strategy_adds[0];deposits=[];totals={'x':0,'y':0}
        for bid,b in start['bins'].items():
            terminal=end['bins'][bid]
            supply=terminal['supply']-b['supply']
            x=terminal['x']-b['x'];y=terminal['y']-b['y']
            if min(supply,x,y)<0:
                raise Unavailable('dlmm_add_liquidity_by_strategy2_negative_delta')
            if not supply:
                if x or y:
                    raise Unavailable('dlmm_add_liquidity_by_strategy2_inventory_without_share')
                continue
            numeric=int(bid)
            if not item['min_bin']<=numeric<=item['max_bin']:
                raise Unavailable('dlmm_add_liquidity_by_strategy2_delta_outside_strategy_range')
            if (x and y) or (x and numeric<=item['active']) or (y and numeric>=item['active']):
                raise Unavailable('dlmm_add_liquidity_by_strategy2_bin_composition')
            if not x and not y:
                raise Unavailable('dlmm_add_liquidity_by_strategy2_share_without_inventory')
            if dlmm.deposit_share(b,x,y)!=supply:
                raise Unavailable('dlmm_add_liquidity_by_strategy2_share_delta_mismatch')
            deposits.append(dict(
                bin_id=numeric,x=x,y=y,observed_share=supply))
            totals['x']+=x;totals['y']+=y
        if totals['x']!=item.get('amount_x') or totals['y']!=item.get('amount_y'):
            raise Unavailable('dlmm_add_liquidity_by_strategy2_total_mismatch')
        if not deposits and (item.get('amount_x') or item.get('amount_y')):
            raise Unavailable('dlmm_add_liquidity_by_strategy2_empty_distribution')
        item['bin_deposits']=deposits
        return
    if removals and adds:
        # Direct-add distributions are exact from their authenticated
        # instruction/event/transfer. They may coexist with removals only when
        # their bin ranges are disjoint, keeping supply attribution unique.
        for removal in removals:
            lower,upper=removal['lower'],removal['upper']
            for add in adds:
                if any(lower<=row['bin_id']<=upper
                       for row in add.get('bin_deposits') or ()):
                    raise Unavailable('dlmm_mixed_liquidity_overlap')
    if not removals:
        if adds:
            return
    ...
    for bid,b in start['bins'].items():
        ...
        if delta<0:
            raise Unavailable('dlmm_liquidity_supply_increase_in_remove_range')
        if delta:
...
def apply_external_adjustment(state,item,counterfactual=False):
    ...
    if kind=='claim_fee2':
        ...
        if kind=='add_liquidity_one_side' and (
                item.get('token_mint')!=result['y']
                or item.get('reserve')!=result['vault_y']
                or result.get('token_y_program')!=pump.TOKEN_PROGRAM):
            raise Unavailable('dlmm_add_liquidity_one_side_token_identity')
        totals={'x':0,'y':0};tracked=0
        for row in deposits:
            bid=row.get('bin_id');x=row.get('x');y=row.get('y')
            if type(bid) is not int or type(x) is not int or type(y) is not int \
                    or min(x,y)<0 or (x and y):
                raise Unavailable('dlmm_add_liquidity2_deposit_shape')
            totals['x']+=x;totals['y']+=y
            b=result['bins'].get(str(bid))
            if b is None:
                if kind!='add_liquidity_one_side':
                    raise Unavailable('dlmm_add_liquidity2_bin_not_observed')
                continue
            tracked+=1
            share=dlmm.deposit_share(b,x,y)
            if share<=0:
                raise Unavailable('dlmm_add_liquidity_zero_share')
            observed=row.get('observed_share')
            if (not counterfactual and observed is not None
                    and (type(observed) is not int or share!=observed)):
                raise Unavailable('dlmm_add_liquidity_observed_share_mismatch')
            b['x']+=x;b['y']+=y;b['supply']+=share
        if totals['x']!=item.get('amount_x') or totals['y']!=item.get('amount_y'):
            raise Unavailable('dlmm_add_liquidity2_total_mismatch')
        if kind=='add_liquidity_one_side' and not tracked:
            raise Unavailable('dlmm_add_liquidity_one_side_no_tracked_bins')
        result['vault_x_amount']+=totals['x'];result['vault_y_amount']+=totals['y']
    else:
...
def reconstruct(start,end_snapshot,signatures,transactions,now,cursor):
    ...
    strategy_effects=[
        item for item in all_effects
        if item.get('kind')=='add_liquidity_by_strategy2']
    if strategy_effects and any(swaps for swaps,_effects in parsed.values()):
        raise Unavailable('dlmm_add_liquidity_by_strategy2_mixed_with_swap_interval')
