...
- public Solana WebSocket plus authenticated OnFinality WebSocket form a deduplicated
  finalized DLMM-program signature union;
...
class UnionSignatureLedger:
    ...
    def observe(self,provider,signature,slot,observed_at):
    ...
    def rows(self):
        return [
            dict(signature=row["signature"],slot=row["slot"],
...
def discovery_streams(environ=None):
    """Return redundant DLMM discovery streams without exposing credentials."""
    rows=[(PUBLIC_DISCOVERY_PROVIDER,PUBLIC_SOLANA_WS_URL)]
    try:
        auth=primary_ws_url(environ,require_authenticated=True)
...
def collect_program_union(duration_seconds,*,environ=None,require_public=True):
    ...
    return dict(
        ...
        signatures=ledger.rows(),union=ledger.status(),
